...
def delete_bookmark(request, bookmark_id):
    try:
        ...
        bookmark = get_object_or_404(
            Bookmark, restaurant__id=bookmark_id, user=request.user
        )
        bookmark.delete()
        return JsonResponse(
        ...
        return JsonResponse(
            {
                "status": "error",
                "message": "Bookmark not found or does not belong to the user.",
            },
            status=404,
        )
        ...
        return JsonResponse(
            {"status": "error", "message": f"Failed to remove bookmark: {str(e)}"},
            status=500,
        )
